...
public class IndexStatsChangingNumberOfMembersTest extends HazelcastTestSupport {
    ...
    /**
     * Note: this test counts with situation where inserts, updates, removes stats are not recalculate after adding/removing a
     * new member; in case when they should be recalculate then this test should be rewritten.
     */
    @Test
    public void testIndexStatsOperationChangingNumberOfMembers() throws InterruptedException {
        ...
        assertEquals(0, valueStats(map1).getEntryCount());
        ...
        assertEquals(0, valueStats(map2).getEntryCount());
        ...
        assertEquals(inserts - removes, valueStats(map1).getEntryCount() + valueStats(map2).getEntryCount());
        ...
        assertEquals(inserts - removes,
                valueStats(map1).getEntryCount() + valueStats(map2).getEntryCount() + valueStats(map3).getEntryCount());
        // this should be changed based on Sergey's answers - it depends whether insert count etc. should be the same or not
        ...
        assertEquals(2 * (inserts - removes), valueStats(map1).getEntryCount() + valueStats(map2).getEntryCount() + valueStats(map3).getEntryCount());
        ...
        assertEquals(2 * (inserts - removes), valueStats(map1).getEntryCount() + valueStats(map3).getEntryCount());
        // this should be changed based on Sergey's answers - it depends whether insert count etc. should be the same or not
        assertEquals(originalMap1InsertCount, valueStats(map1).getInsertCount());
        assertEquals(originalMap1UpdateCount, valueStats(map1).getUpdateCount());
        assertEquals(originalMap1RemoveCount, valueStats(map1).getRemoveCount());
        ...
        assertEquals(3 * (inserts - removes), valueStats(map1).getEntryCount() + valueStats(map3).getEntryCount());
    ...
    private LocalMapStats stats(IMap map) {
        return map.getLocalMapStats();
    }

    private LocalIndexStats valueStats(IMap map) {
